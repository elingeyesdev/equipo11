    ...
    <div style={{ overflowX: 'auto', overflowY: 'hidden', paddingBottom: '8px', width: '100%' }} className="rep-chart-scrollable">
      <svg viewBox={`0 0 ${W} ${H}`} style={{ minWidth: W, height: H, display: 'block' }} aria-hidden="true">
        {[0, 0.5, 1].map((t, i) => {
          ...
          return (
            <g key={i}>
              <line x1={PAD.l} x2={W - PAD.r} y1={y} y2={y} stroke="var(--border-color)" strokeDasharray="3,3" strokeWidth={0.8} />
              <text x={PAD.l - 4} y={y + 3.5} textAnchor="end" fontSize={9} fill="var(--text-secondary)">{(maxV * t).toFixed(0)}</text>
            </g>
          );
        })}
        {cities.map((c, i) => {
          ...
          return (
            <g key={c.name}>
              ...
              <text x={x + bW / 2 + 4} y={H - PAD.b + 16} textAnchor="end" fontSize={9} fill="var(--text-secondary)" transform={`rotate(-45, ${x + bW / 2 + 4}, ${H - PAD.b + 16})`}>
                {c.name.length > 15 ? c.name.slice(0, 13) + '…' : c.name}
              </text>
              <text x={x + bW / 2} y={y - 4} textAnchor="middle" fontSize={8} fill="var(--text-secondary)" fontWeight="600">
                {c.avg.toFixed(1)}
              </text>
            </g>
          );
