...
export default function FronterasPanel({ onBoundarySelect, onStartSimulation, isRunning }) {
  ...
  const handleStart = () => {
    if (!zona1.result) {
      return addToast("Selecciona al menos una zona válida (País/Departamento/Provincia o Manual).", 'warning')
    }
    if (isComparing) {
      if (!zona2.result) {
        return addToast("Selecciona la segunda zona para comparar.", 'warning')
      }
      if (zona1.selectionMode === 'country' && zona2.selectionMode === 'country' && zona1.pais && zona2.pais && zona1.pais === zona2.pais) {
        return addToast("No puedes seleccionar el mismo país para la comparación.", 'warning')
      }
    }
  ...
  const renderFronteraForm = (z, setZ, isZ2) => (
    <div className="fp-zona-box">
      <div className="fp-mode-tabs">
        <button
          type="button"
          className={`fp-mode-tab ${z.selectionMode === 'country' ? 'active' : ''}`}
          disabled={isRunning}
          onClick={() => {
            const nextZ = { ...z, selectionMode: 'country', result: null };
            setZ(nextZ);
            if (activeDrawingZone === (isZ2 ? 'z2' : 'z1')) setActiveDrawingZone(null);
            emitBoundaries(isZ2 ? zona1 : nextZ, isZ2 ? nextZ : zona2, isZ2 ? 'z2' : 'z1');
          }}
        >
          País/Zona
        </button>
        <button
          type="button"
          className={`fp-mode-tab ${z.selectionMode === 'manual' ? 'active' : ''}`}
          disabled={isRunning}
          onClick={() => {
            const nextZ = { ...z, selectionMode: 'manual', result: updateManualResult(z.manualPoints, isZ2, z.manualName) };
            setZ(nextZ);
            setActiveDrawingZone(isZ2 ? 'z2' : 'z1');
            emitBoundaries(isZ2 ? zona1 : nextZ, isZ2 ? nextZ : zona2, isZ2 ? 'z2' : 'z1');
          }}
        >
          Manual (Puntos)
      ...
      {z.selectionMode === 'country' ? (
        <>
          <div className="fp-field">
            ...
            <SearchableComboBox
              ...
              options={paises.map(p => p.name).filter(name => {
                const other = isZ2 ? zona1 : zona2;
                if (isComparing && other.selectionMode === 'country') {
                  return name !== other.pais;
                }
                return true;
              })}
        ...
        <div className="fp-manual-container">
          <div className="fp-field" style={{ marginBottom: '12px' }}>
            ...
            <input
              ...
              disabled={isRunning}
              value={z.manualName || ''}
              placeholder={isZ2 ? "Zona Manual 2" : "Zona Manual 1"}
              onChange={e => {
                ...
                const nextZ = {
                  ...
                  result: z.result ? {
                    ...z.result,
                    nombre: val || (isZ2 ? "Zona Manual 2" : "Zona Manual 1"),
                    geojson: {
                      ...z.result.geojson,
                      features: [{
                        ...z.result.geojson.features[0],
                        properties: { name: val || (isZ2 ? "Zona Manual 2" : "Zona Manual 1") }
                      }]
                    }
                  } : null
                };
          ...
          <div className="fp-manual-status">
            <span>Puntos marcados: <strong>{z.manualPoints.length}</strong></span>
            {z.manualPoints.length < 3 && <span className="fp-msg-warn"> (mínimo 3 para cerrar la zona)</span>}
          </div>
          <div className="fp-manual-actions">
            <button
              type="button"
              className={`fp-btn-draw ${activeDrawingZone === (isZ2 ? 'z2' : 'z1') ? 'drawing' : ''}`}
              onClick={() => setActiveDrawingZone(activeDrawingZone === (isZ2 ? 'z2' : 'z1') ? null : (isZ2 ? 'z2' : 'z1'))}
            >
              {activeDrawingZone === (isZ2 ? 'z2' : 'z1') ? 'Detener dibujo' : 'Dibujar'}
            </button>
            <button
              type="button"
              className="fp-btn-clear"
              disabled={z.manualPoints.length === 0}
              onClick={() => {
                const nextZ = { ...z, manualPoints: [], result: null };
                setZ(nextZ);
                emitBoundaries(isZ2 ? zona1 : nextZ, isZ2 ? nextZ : zona2, isZ2 ? 'z2' : 'z1');
              }}
            >
