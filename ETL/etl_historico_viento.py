        ...
        for month in MONTHS:
            ...
            try:
                ...
                for i in range(total_hours):
                    ...
                    u_var = [v for v in list(ds.data_vars) if v.startswith('u')][0]
                    v_var = [v for v in list(ds.data_vars) if v.startswith('v')][0]
                    u_frame = np.flipud(ds[u_var].isel({time_dim: i}).values)
                    v_frame = np.flipud(ds[v_var].isel({time_dim: i}).values)
